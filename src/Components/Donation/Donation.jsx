...
function Donation() {
    ...
    return (
        <div className='font-sans'>
            <div
                className='relative min-w-full xl:h-[80vh] h-[40vh] mx-auto overflow-y-hidden object-cover'
            >
                <img
                    src={DonationPage}
                    alt='Donation'
            ...
            <div className='px-4 pt-10 mb-5 md:px-14 md:py-8 flex flex-col justify-start'>
                ...
                <input type="text" className='w-full bg-gray-200 outline-none px-5 py-2 rounded-[10px] placeholder-gray-400' placeholder='Search for where you want to donate eg: child education, health centers etc.' />
            </div>
            <div>
                {donations.map((item,index)=>{
                    return(
                        <Funds key={index} img={item.img} title={item.title} description={item.description} createdBy={item.createdBy} amount={item.amount} progress={item.progress} target={item.target} />
                    )
                })}
            </div>
        </div>
